...
__usage() {
    ...
    echo "                   Example: ./flink.sh flinkjob --project your-gcp-project \\"
    echo "                                            --subscription your-pubsub-subscription \\"
    echo "                                            --saCredentials gs://your-bucket/your-sa-key.json \\"
    echo "                                            --gcsStateBackendPath gs://your-flink-bucket/checkpoints \\"
    echo "                                            [--useGcpPubsubConnectors true] \\"
    echo "                                            [--checkpointInterval 60000]"
    echo ""
...
__flink_job_server() {
    ...
    gcloud dataproc clusters create $FLINK_JOB_CLUSTER_NAME \
        ...
        --properties=flink:historyserver.archive.fs.dir=$FLINK_GCS_ROOT/$FLINK_JOB_CLUSTER_NAME/flink-job-history \
        --properties=flink:historyserver.archive.fs.refresh-interval=3000 \
        --properties=flink:historyserver.web.refresh-interval=3000 \
        --properties=flink:jobmanager.archive.fs.dir=$FLINK_GCS_ROOT/$FLINK_JOB_CLUSTER_NAME/flink-job-history/completed-jobs \
